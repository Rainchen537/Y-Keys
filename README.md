...
1. 启动 Y-Keys 后，先切换到想查询快捷键的 App，例如访达或文本编辑。
2. 快速按下并松开两次**左侧 Command（⌘）**，呼出快捷键面板。
3. 按住 `⌘`、`⌥`、`⌃` 或 `⇧`，查看与当前修饰键组合兼容的高亮项。
4. 点击面板外区域关闭；使用 Esc 时，如果面板提示“再次按下 Esc 退出”，请在 1.2 秒内再按一次。

Y-Keys 常驻菜单栏，不显示 Dock 图标。需要检查权限或更新时，从菜单栏图标进入“打开设置”。
...
触发键是**左侧 Command**，请快速连按并松开两次。打开菜单栏中的设置，在“权限”页检查辅助功能、输入监控与键盘监听三项状态；授权步骤见上方“权限说明”。若两项权限均已开启但监听仍未运行，按权限页提示重启正式安装版。

### 为什么有些 App 快捷键没有显示

应用快捷键来自目标 App 通过辅助功能接口公开的菜单项。仅存在于 App 内部、没有通过菜单暴露的快捷键可能无法读取；可先查看该 App 自己的菜单或帮助文档。查询前先切换到目标 App，让 Y-Keys 记录当前查询对象。

### 修改系统快捷键后，面板为什么仍显示原来的组合

当前系统分组使用内置快捷键清单，尚未读取系统中用户自定义的快捷键配置。自定义组合请以 macOS 系统设置中的实际配置为准。
